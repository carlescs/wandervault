...
private enum class TripDetailTab(@StringRes val labelRes: Int, val icon: ImageVector) {
    ...
    DOCUMENTS(R.string.trip_detail_tab_documents, Icons.Default.Description),
}
...
internal fun TripDetailContent(
    ...
    Scaffold(
        ...
        when (selectedTab) {
            ...
            TripDetailTab.DOCUMENTS -> DocumentsTabContent(
                documents = (uiState as? TripDetailUiState.Success)?.documents ?: emptyList(),
                innerPadding = innerPadding,
...
/**
 * Content for the Documents tab of the Trip Detail screen.
 *
 * Shows all documents for the trip grouped by folder. Within each folder, documents are listed
 * with their name and MIME type. A delete action is available on each document.
 */
@Composable
private fun DocumentsTabContent(
    ...
    if (documents.isEmpty()) {
        ...
        // Group documents by folder, placing null-folder docs under the catch-all group.
        val generalFolderLabel = stringResource(R.string.documents_folder_general)
        val grouped = documents.groupBy { it.folder ?: generalFolderLabel }
        LazyColumn(
            modifier = modifier.fillMaxSize(),
            contentPadding = innerPadding,
        ) {
            grouped.entries.forEach { (folder, docs) ->
    ...
    documentToDelete?.let { doc ->
        AlertDialog(
            ...
            title = { Text(stringResource(R.string.documents_delete_dialog_title)) },
            text = { Text(stringResource(R.string.documents_delete_dialog_message, doc.name)) },
            confirmButton = {
                TextButton(onClick = {
                    onDeleteDocument(doc.id)
                    documentToDelete = null
                }) {
                    Text(stringResource(R.string.documents_delete_dialog_confirm))
                }
            },
            dismissButton = {
...
private fun DocumentRow(
    ...
    Row(
        ...
        IconButton(onClick = onDelete) {
            Icon(
                imageVector = Icons.Default.Delete,
                contentDescription = stringResource(R.string.documents_delete_document),
            )
        }
    }
}
