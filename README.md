...
WanderVault can back up and restore the database to/from Google Drive.  To enable this feature you must register the app as an OAuth 2.0 client in the Google Cloud Console.

### 1. Create (or select) a Google Cloud project

1. Open [Google Cloud Console](https://console.cloud.google.com/).
2. Click the project selector at the top and choose **New Project** (or pick an existing one).
3. Give the project a name (e.g. `WanderVault`) and click **Create**.
...
2. Choose **External** (or **Internal** if you are using Google Workspace).
3. Fill in the required fields (app name, support email, developer contact) and click **Save and Continue**.
4. On the **Scopes** step click **Add or Remove Scopes**, search for `drive.file`, and add it.
5. Complete the remaining steps and publish the app if needed.
...
3. Enter the package name: `cat.company.wandervault`
4. Enter the **SHA-1 certificate fingerprint** of your signing key.  
   To obtain it for the debug keystore run:
   ```bash
   keytool -list -v \
   ...
   ```
   For a release keystore substitute your own keystore path, alias, and passwords.
5. Click **Create**.  Note the generated **Client ID** — you will not need to embed it in the source; the Google Mobile Services (GMS) SDK resolves it automatically from the package name + SHA-1 at runtime.

### 5. Download and place `google-services.json`

1. On the **Credentials** page click **Download** (⬇) next to your Android client, or go to **Project Settings** if you also set up a Firebase project, and download `google-services.json`.
2. Copy the file into the `app/` directory of the repository:
   ```
   wandervault/
   └── app/
...
> **Template**: A `google-services.json.example` file is provided in `app/` showing the expected JSON structure.  Replace every `YOUR_*` placeholder with the real values from the Cloud Console before renaming it to `google-services.json`.

### 6. Build and run

No code changes are required.  Once `google-services.json` is in place, build the app normally (`./gradlew assembleDebug`) and the Google Sign-In + Drive backup feature will be fully functional.
